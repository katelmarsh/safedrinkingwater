...
write.csv(data, "joined_tx_weather.csv")
...
write.csv(sdwis_tceq, "merged_tceq_sdwis_2015_2020.csv")
...
out <- lm(X..of.Violations ~ X..of.Facilities + X..of.Site.Visits + Population.Served.Count + LAT + LON + FIPS + ZONE + BEGIN_DT + END_DT + VIOL_NUM + POPULATION + VIOLATION_YEAR, data = sdwis_tceq) 
summary(out)
...
max_fips <- aggregate(DAMAGE_PROPERTY_NUM~FIPS, data=data, max, na.rm = TRUE)
which()
which(data$DAMAGE_PROPERTY_NUM == max_fips$DAMAGE_PROPERTY_NUM, arr.ind = TRUE)

library(data.table)
data$FIPS <- as.factor(data$FIPS)
as.data.table(data)[, DAMAGE_PROPERTY_NUM, 
                          by = FIPS]

max_fips <- data[data$DAMAGE_PROPERTY_NUM == ave(data$DAMAGE_PROPERTY_NUM, data$FIPS, FUN=max, na.rm=TRUE),]
